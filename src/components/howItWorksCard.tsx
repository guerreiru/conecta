...
function Step({ children, title, description }: StepProps) {
  return (
    <div className="flex flex-col gap-4">
      <div className="p-2 md:p-3 bg-black w-fit rounded-full self-center">{children}</div>

      <div className="max-w-20 md:max-w-none">
        <p className="text-center text-slate-800 text-sm font-semibold">
          {title}
        </p>
        <p className="mt-1 text-center text-gray-600 text-[10px]">{description}</p>
      </div>
    </div>
  );
}
...
export function HowItWorksCard() {
  return (
    <div className="bg-lime-400 rounded-3xl p-4 text-black grid gap-6">
